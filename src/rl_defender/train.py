"""
Training script for the Multi-turn RL Defender (offline GRPO).

Requires a prepared JSONL dataset produced by data_preparation.py.

Typical usage (LoRA):
  python -m src.rl_defender.train \\
      --prepared-train-data data/rl_defender/grpo_train.jsonl \\
      --base-model          Qwen/Qwen3-4B-Instruct-2507 \\
      --training-type       lora \\
      --output-dir          checkpoints/rl_defender \\
      --num-epochs          3

Multi-GPU with DeepSpeed:
  torchrun --nproc_per_node 4 -m src.rl_defender.train \\
      --prepared-train-data ... \\
...
from src.rl_defender.dataset import GRPODataset
from src.rl_defender.trainer import GRPODefenderTrainer
...
def load_model_and_tokenizer(config: RLDefenderConfig):
    """Load base model and tokenizer (or resume checkpoint for full FT)."""
    ...
    tokenizer.padding_side = "right"  # right-padding keeps prompt_len indexing valid
...
def setup_lora(model, config: RLDefenderConfig):
    """Attach a fresh LoRA adapter, or reload one from a resume checkpoint."""
...
def train_rl_defender(config: RLDefenderConfig) -> str:
    """
    Main training loop.
    ...
    # ── token IDs for "0" (BLOCK) and "1" (PASS) ─────────────────────────
    ...
    if config.eval_epochs_only:
        eval_steps = (
            10**9
        )  # effectively never fires in on_step_end; only on_epoch_end runs
    else:
    ...
    # ── TrainingArguments ─────────────────────────────────────────────────
    # Save strategy: step-level if save_steps is set (better for resume after
    # mid-epoch crashes); otherwise save once per epoch.
    if config.save_steps:
        _save_strategy = "steps"
        _save_steps = config.save_steps
    else:
        _save_strategy = "epoch"
        _save_steps = 500  # ignored when strategy="epoch"
    ...
    training_kwargs = dict(
        ...
        remove_unused_columns=False,  # preserve custom fields
    ...
    # Mirrors the SFT DefenseEvalCallback: runs vLLM-based defense evaluation
    # on the val set every eval_steps, and val+test at each epoch end.
    ...
    # ── train ─────────────────────────────────────────────────────────────
    _log("\n" + "=" * 70)
    _log("Starting offline GRPO training …")
    _log("=" * 70 + "\n")

    # When resuming, pass the checkpoint path so HF Trainer restores the
    # optimizer/scheduler states and skips already-completed steps.
...
def parse_args():
    p = argparse.ArgumentParser(description="Train RL Defender (offline GRPO)")
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="JSONL from data_preparation.py used for training.",
    )
    p.add_argument(
        "--prepared-val-data", type=str, default=None, help="Optional validation JSONL."
    )
    p.add_argument("--output-dir", type=str, default="checkpoints/rl_defender")

    # GRPO
    p.add_argument(
        "--kl-coeff", type=float, default=0.04, help="β: KL divergence penalty weight."
    )
    p.add_argument(
        "--use-is-ratio", action="store_true", help="Apply PPO-style IS ratio clipping."
    )
    p.add_argument(
        "--clip-epsilon",
        type=float,
        default=0.2,
        help="IS ratio clip range ε (used when --use-is-ratio).",
    )
    p.add_argument(
        "--clip-higher",
        action="store_true",
    ...
    p.add_argument(
        "--batch-size", type=int, default=4, help="Per-device training batch size."
    )
    ...
    # Distributed
    p.add_argument(
        "--deepspeed", type=str, default=None, help="Path to DeepSpeed config JSON."
    )
    ...
    p.add_argument(
        "--resume",
        action="store_true",
        help="Resume training from --resume-model-path checkpoint.",
    )
    p.add_argument(
        "--resume-model-path",
        type=str,
        default=None,
        help="HF Trainer checkpoint dir to resume from "
        "(e.g. checkpoints/rl_defender_lora/checkpoint-500). "
        "Restores model weights, optimizer/scheduler states and "
        "skips already-completed steps.",
    )
    p.add_argument(
        "--resume-init-eval",
        action="store_true",
        help="Run val+test defense eval on the resumed model before "
        "training starts (requires --eval-data-dir).",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Dir with pre-generated eval conversations "
        "(val_conversations.pkl / test_conversations.pkl). "
        "Enables periodic defense evaluation via vLLM subprocess.",
    )
    p.add_argument(
        "--eval-gpu",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Defense eval every N optimizer steps "
        "(default: steps_per_epoch // 10).",
    )
    p.add_argument(
        "--eval-epochs-only",
        action="store_true",
        help="Only run defense eval at epoch end; skip mid-epoch step evals.",
    )
    p.add_argument(
        "--defense-batch-size",
        type=int,
    ...
    p.add_argument(
        ...
        "Default (None) saves once per epoch. "
        "Use a smaller value for finer-grained resume points.",
    )
