...
class JailbreakExperiment:
    ...
    def setup_model(self) -> None:
        ...
        try:
            ...
            if defense_config:
                d_type = defense_config.get("type", "none")
                if d_type == "intent_defender":
                    # intent_defender is Output Defense handled at CKA-Agent level,
                    # not Input Defense at ModelLoader level — skip BaseDefense creation
                    self.logger.info(
                        "Defense type is 'intent_defender' (Output Defense), "
                        "handled by CKA-Agent — skipping ModelLoader defense setup"
                    )
                else:
                    model_config_with_defense["defense"] = defense_config
                    self.logger.info(
                        f"Defense enabled: {defense_config.get('enabled', False)}, type: {d_type}"
                    )
            ...
            if defense_config.get("enabled", False):
                ...
                if d_type == "llm_guard":
                    ...
                    # Pure perturbation requires no GPU, skip
                    defense_model_name = None
    ...
    def _run_method_on_dataset_resume(
        ...
        # Aggregate and print Intent-Defender defense stats if present
        defense_results = [
            r["metadata"]["defense_stats"]
            for r in results
            if r.get("metadata", {}).get("defense_stats")
        ]
        if defense_results:
            total_checked = sum(d["total_checked"] for d in defense_results)
            total_blocked = sum(d["blocked"] for d in defense_results)
            total_passed = sum(d["passed"] for d in defense_results)
            overall_block_rate = (
                total_blocked / total_checked if total_checked > 0 else 0.0
            )
            self.logger.info(
                f"\n{'='*50}\n"
                f"  Intent-Defender Online Defense Summary\n"
                f"{'='*50}\n"
                f"  Queries defended:  {len(defense_results)}\n"
                f"  Total nodes checked: {total_checked}\n"
                f"  Blocked: {total_blocked}\n"
                f"  Passed:  {total_passed}\n"
                f"  Overall block rate:  {overall_block_rate:.2%}\n"
                f"{'='*50}"
            )
    ...
    def _allocate_method_gpus(
        ...
        if method_name == "cka-agent":
            ...
            elif judge_config.get("type") == "blackbox":
                # Blackbox judge uses API, no GPU allocation needed
                pass
    ...
    def _load_method(
        ...
        try:
            ...
            # Inject defense configs from defense section into method config
            defense_cfg = self.config.get("defense", {})
            if defense_cfg.get("enabled", False):
                # Intent-Defender (SFT/RL, terminates search)
                if (
                    "intent_defender" not in method_cfg
                    and defense_cfg.get("type") == "intent_defender"
                ):
                    intent_defender_cfg = defense_cfg.get("intent_defender")
                    if intent_defender_cfg:
                        method_cfg["intent_defender"] = intent_defender_cfg

                # Input filter (e.g. intention_analysis, before target)
                input_filter_cfg = defense_cfg.get("input_filter")
                if input_filter_cfg and "input_filter" not in method_cfg:
                    method_cfg["input_filter"] = input_filter_cfg

                # Output filter (e.g. qwen_guard, after target)
                output_filter_cfg = defense_cfg.get("output_filter")
                if output_filter_cfg and "output_filter" not in method_cfg:
                    method_cfg["output_filter"] = output_filter_cfg
    ...
    def _run_method_on_dataset(
        ...
        try:
            if hasattr(method, "generate_jailbreak_batch"):
                ...
                if batch_size > 1:
                    # Process in batches (dataset batch processing)
                    self.logger.info(
